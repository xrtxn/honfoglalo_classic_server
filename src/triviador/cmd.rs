...
impl Cmd {
	...
	pub(crate) async fn get_player_cmd(
		...
		let available;
		if GameState::get_gamestate(temp_pool, game_id).await?.state == 1 {
			available = AvailableAreas::get_available(temp_pool, game_id).await;
		} else {
			available =
				AvailableAreas::get_limited_available(temp_pool, game_id, rel_player_id).await;
		}
